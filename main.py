...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    layer7_out = tf.layers.conv2d_transpose(conv_1x1_layer7,
	    					  num_classes,
	    					  4, # kernel_size
	    					  strides= (2, 2),
	    					  padding= 'same',
    ...
    # Adding skip layer.
    first_skip = tf.add(layer7_out, conv_1x1_layer4, name='first_skip')
    
    # Upsample deconvolutions
    layer4_out = tf.layers.conv2d_transpose(first_skip,
                                                   num_classes,
                                                   4, # kernel_size
                                                   strides= (2, 2),
                                                   padding= 'same',
    ...
    # Adding skip layer.
    second_skip = tf.add(layer4_out, conv_1x1_layer3, name='second_skip')
    
    # Upsample deconvolution
    last_layer = tf.layers.conv2d_transpose(second_skip, num_classes, 16,
                                                  strides= (8, 8),
                                                  padding= 'same',
                                                  kernel_initializer= tf.random_normal_initializer(stddev=0.01),
                                                  kernel_regularizer= tf.contrib.layers.l2_regularizer(weights_regularized_l2),
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    print('Starting training... Total {} epochs'.format(epochs))
    print()
